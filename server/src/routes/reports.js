...
router.get('/area-stats', authenticate, asyncHandler(async (req, res) => {
  // 1. Total area per building from rooms
  ...
  // 2. Rented area from active rent contracts' rent_objects
  const contractRes = await pool.query(`
    SELECT e.id, e.properties->>'rent_objects' AS rent_objects
    FROM entities e
    JOIN entity_types et ON e.entity_type_id = et.id AND et.name IN ('contract','supplement')
    WHERE e.deleted_at IS NULL
      AND (e.properties->>'contract_type' IN ('Аренды','Субаренды'))`);

  // For each contract, get latest supplement rent_objects, else contract's own
  const contractIds = new Set();
  const contractMap = {};
  contractRes.rows.forEach(r => {
    contractMap[r.id] = r.rent_objects;
  });
