...
eq_bld AS (
  SELECT DISTINCT ON (equipment_id) equipment_id, building_name FROM (
    ...
    JOIN entity_types bet ON bet.id = b.entity_type_id AND bet.name IN ('building')
...
eq_info AS (
  ...
  LEFT JOIN entity_types bet ON bet.id = b.entity_type_id AND bet.name IN ('building')
  WHERE eq.deleted_at IS NULL
)
SELECT
  af.act_id,
