...
В проекте одновременно могут работать несколько AI-агентов (main + indparkdocs).
Чтобы не затирать работу друг друга — **строгое правило feature-веток**.

### Кто что делает:
| Агент | Бот | Задачи |
|-------|-----|--------|
| main | @Valentina_openclaw_bot (Opus) | Архитектура, безопасность, инфраструктура, сложные задачи |
| indparkdocs | @IndParkDocs_bot (Sonnet) | Фичи, баги, UI, рутинная разработка |

### Правила координации:
1. **Каждый агент работает в своей feature-ветке** — никогда не в `dev` напрямую
2. **Перед началом работы** — `git pull origin dev` и создать `feature/название`
3. **Имена веток**: `feature/agent-main-название` или `feature/agent-indparkdocs-название`
4. **Merge через PR** — это защищает от конфликтов
5. **Не редактировать один файл одновременно** — если нужно, координируй через Valentina

### Branch protection (уже настроено на GitHub):
- `main`: запрещены прямые пуши, обязательный PR + CI зелёный → автодеплой
- `dev`: запрещены force push и удаление ветки
...
1. **НИКОГДА** не хранить массивы доменных объектов в JSON-строках внутри `properties`. Любой массив бизнес-записей (строки договора, авансы, объекты аренды, список оборудования) — это отдельная таблица. При проектировании новой фичи с коллекцией объектов — сначала предложить схему таблицы, потом писать код.
   - ✅ **Можно в `properties`:** скалярные значения (строки, числа, даты, булевы)
   - ❌ **Нельзя в `properties`:** `[{...}, {...}]` — если это бизнес-данные для хранения/фильтрации/агрегации
   - ✅ **Исключения:** `floor_plans` (нативный jsonb для SVG), `map_points` (UI-оверлей), `f.options` (конфиг справочника)
