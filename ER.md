...
graph TD
    ...
    %% ─── Иерархия: стрелка от родителя к потомку ───
    CO  -->|"владеет"| BLD
    LP  -->|"содержит"| LPP
    BLD -->|"содержит"| RM
    CT  -->|"содержит"| SP
...
### Иерархия оборудования (отдельная цепочка parent_id)

Оборудование может быть родителем другого оборудования —
для описания агрегатов из узлов и запчастей:

```
⚙️ Козловой кран               ← агрегат (parent_id = корпус или null)
  └─ ⚙️ Двигатель крана        ← узел    (parent_id = кран)
       └─ ⚙️ Редуктор          ← запчасть (parent_id = двигатель)

⚙️ Трансформатор               ← агрегат
  └─ ⚙️ Обмотка ВН             ← узел
```

> На диаграмме выше петля `оборудование → оборудование` убрана для наглядности.
> Расположение оборудования (в каком корпусе/помещении стоит) задаётся
> через связь `расположен в`, а не через parent_id.
