...
## Задеплоенный коммит
`8c9e3e3` (main) — fix: red highlighting for emergency/broken equipment everywhere (card + registry) (2026-02-25, 18:39 UTC)
...
- **AGG_HIERARCHY_FIELDS** — обсуждение какие поля убрать/добавить в группировке
- **Группировка строк — новый UX**: таблица не перестраивается в дерево; выбранное поле становится фильтр-хедером над таблицей с дропдауном уникальных значений — фильтрует строки по выбранному значению. Нужно продумать взаимодействие с текущим `AGG_HIERARCHY_FIELDS` механизмом.
