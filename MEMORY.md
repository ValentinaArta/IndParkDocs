...
## Задеплоенный коммит
`5da28f3` (main) — fix: migration 017 - use name_ru instead of label column (2026-02-26)
...
## 🚧 Незавершённые задачи (сессия 2026-02-26)

### Собственник на карточке ЗУ — НЕ ИСПРАВЛЕНО
- **Проблема**: balance_owner введён в форме, не отображается на карточке
- **Причина**: карточка читает `props.balance_owner`, сохраняется `balance_owner_id`+`balance_owner_name`
- **Исправление**: на карточке использовать `balance_owner_name`
- **PROPERTY_LABELS** строка ~3671: `balance_owner: 'Собственник'`

### parent_id в форме ЗУ — НЕ ИСПРАВЛЕНО  
- **Проблема**: поле "Входит в (родительский объект)" показывается в форме land_plot
- **Исправление**: добавить `land_plot` в список исключений (строка ~4352–4356 else-ветка)

## 🚧 В очереди (не реализовано)
- **Orphan rooms**: ✅ решено 2026-02-26 — Валентина назначила корпуса всем трём помещениям
- **Анализ аренды**: ✅ ПОЛНОСТЬЮ РЕАЛИЗОВАН (все 22 функции, backend endpoint, тест через Отчёты → вкладка "Анализ аренды")
- **workshop/crane_track/document** — убраны из меню ✅ (2026-02-26, Валентина подтвердила)
- **document.doc_type** — убрать типы которые уже есть как сущности  
- **Карта: полигоны (ПРЕРВАНО)** — задача начата 2026-02-26, НЕ закончена:
  - Цель: SVG overlay вместо CSS div; прямоугольники drag, полигоны click+dblclick
  - Данные: полигоны → `map_shape:'polygon'` + `map_points:'[[x,y],...]'` + `map_color`
  - Секция карты: строки ~2247–2515 в frontend.js
  - Нужно заменить весь блок `// ============ INTERACTIVE MAP ============`
- **BI/Metabase** — когда база наполнится данными
- **land_plot_part** — добавить в ER диаграмму
- **🔍 Поиск по субарендатору** — `subtenant_name` не в SQL поиска; "Интилед" не находится
- **🔢 Автономер ДС** — max+1 при открытии формы
- **🏛 Скрыть parent_id для компаний** — нет семантики
- **🏢 Inline-создание компании из договора** — "Другое..." → мини-форма компании
- **📊 Структура владения** *(ER → согласование)* — поле для своих юрлиц
- **👤 Разные контакты** *(ER → согласование)* — свои: ГД; сторонние: multi-contact
- **📋 Предметы договора услуг** *(ER → согласование)* — multiple rows per contract
- **🚫 Защита от дублей компаний** — fuzzy search + INN check + merge tool
- **⏳ Индикатор загрузки (spinner)** — показывать крутилку при любом async-вызове перед открытием модалки/формы, чтобы пользователь видел что сайт "думает"
- **🔎 Поиск при выборе контрагента** — заменить `<select>` на searchable dropdown (input + фильтруемый список): пользователь вводит буквы, список сужается. Применить ко всем полям выбора компании в формах договоров (`renderRegistrySelectField` для company)
- **📊 Ставка НДС в ДС** — поле "в т.ч. НДС, %" должно быть во всех формах доп. соглашений (любой тип договора), чтобы ставку можно было изменить через ДС
- **🌍 Quick-create ЗУ из формы корпуса** — при нажатии "+ Создать ЗУ" открывать полную форму земельного участка (все поля), а не только запрос кадастрового номера
- **📐 Площадь ЗУ — сменить единицу с "га" на "кв.м."** (поле label в field_definitions для land_plot)
- **🏛 Собственник корпуса/ЗУ — только наши организации + поиск** — в поле "Собственник" для building и land_plot показывать только компании с is_own=true; добавить поиск при выборе (как для контрагентов в договорах)
- **🛒 Договор купли-продажи — предмет и связи** *(ER → согласование)*:
  - Новый справочник "Типы предмета договора купли-продажи" (Оборудование / Корпуса / Земельные участки)
  - Поле "Предмет" в форме купли-продажи: выбор из справочника
  - При выборе типа → inline-создание соответствующей сущности (оборудование / корпус / земельный участок)
  - Поле "Поставщик" на карточке оборудования = автоматически из договора купли-продажи этого оборудования
  - Новые связи: договор купли-продажи → оборудование (purchased_in), договор → компания-продавец (seller)
