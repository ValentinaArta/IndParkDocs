...
## Задеплоенный коммит
`719e7ae` (main), `980c31b` (dev) — fix: remove duplicate eq name level when eq_name already in hierarchy (2026-02-25 вечер)
...
## 🚧 В обсуждении / Очередь задач (не реализовано)
- **Карточка договора аренды — новый вид** (2026-02-25):
  - Заголовок: "Арендатор, №договора, Дата"
  - Арендодатель (our_legal_entity)
  - Помещения: список описаний помещений из реестра (room.properties.description)
  - Таблица: Название помещения | Площадь | Ставка из последнего ДС
  - Ежемесячный платёж: сумма (площадь × ставка) по всем помещениям
  - Срок действия: contract_end_date
  - Переданное оборудование (раскрывающийся): equipment_list последнего ДС (transfer_equipment=true) — показать: название, тип, место установки (красный если Аварийное)
  - История ДС (раскрывающийся): начиная с договора → все ДС по дате: №, дата, changes_description
  - Требует нового backend endpoint `GET /api/contracts/:id/card` (включает located_in relation для оборудования)
- **AGG_HIERARCHY_FIELDS** — обсуждение какие поля убрать/добавить в группировке (в процессе на 2026-02-25)
- **Группировка строк — новый UX** (2026-02-25): таблица не перестраивается в дерево; выбранное поле становится фильтр-хедером над таблицей с дропдауном уникальных значений — фильтрует строки по выбранному значению. Нужно продумать взаимодействие с текущим `AGG_HIERARCHY_FIELDS` механизмом.
- **Анализ аренды** — JS функции не завершены (buildRentAnalysis, renderRentTable, фильтры)
- **Акт — визуальные улучшения**: блоки позиций с разным фоном, "Нерабочий/аварийный" на одной строке
- **AGG_CONTRACT_TYPES** — добавить `checked` по умолчанию
- **Фильтр комнат из отчётов по оборудованию** — LEFT JOIN → INNER JOIN в reports.js
- **Resize модалок** — setModalContent/setModalSize
...
- Перед пушем: `node -e "require('./server/src/frontend.js'); console.log('OK')"`

## Планы / Next Steps
- Дописать JS функции для "Анализ аренды"
- Проверить подсветку аварийного оборудования на живом сайте (verify крейны подсвечены)
- Добавить оборудование в БД и проверить отчёты "По связям"
