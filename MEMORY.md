...
## Задеплоенный коммит
`b9ab35e` (main) — docs: sidebar cleanup confirmed done (2026-02-26)
...
- **Интерактивная карта территории** — SVG hotspot overlay (ждём картинки от Валентины)
...
- **НИКОГДА не добавлять новые entity types или lists без согласования схемы!**
  Если задача требует новой сущности/списка → сообщить что именно нужно изменить → прислать обновлённую схему → ждать ОК → только потом код.
